...
{ 
  ...
  const u_short  srcPort = ntohs(sender.sin_port);
  ...
  case PKT_REQUEST_JOIN: {
    ...
    for (int n = 0; n < players.len; n++) {
      ...
      const char    *pAddr = inet_ntoa(currP.ipAddr.sin_addr);
      const u_short  pPort = ntohs(currP.ipAddr.sin_port);

      if (strcmp(pAddr, ipAddr) != 0 || pPort != srcPort)
        continue;
